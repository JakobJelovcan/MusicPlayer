...
namespace ExtensionsLibrary.Extensions
{
    public static class IListExtensions
    {
        ...
        public static void AddIfDoesntContainInAscendingOrder<T>(this IList<T> target, T item, params Func<T, IComparable>[] selectors)
        {
            if (item is not T) throw new ArgumentNullException();
            if (target.Any() && !(target?.Contains(item) ?? false))
            {
                int index = target.BinarySearch(item, selectors);
                if (index != -1)
                {
        ...
        public static void AddIfDoesntContainInDescendingOrder<T>(this IList<T> target, T item, params Func<T, IComparable>[] selectors)
        {
            if (item is not T) throw new ArgumentNullException();
            if (target.Any() && !(target?.Contains(item) ?? false))
            {
        ...
        public static void UpdateItemInDescendingOrder<T>(this IList<T> collection, T item, params Func<T, IComparable>[] selectors)
        {
            ...
            if (collection.Any())
            {
                ...
                if (index >= 0)
                {
                    if (!item.Equals(collection[index]))
                    {
                        collection.Remove(item);
                        collection.Insert(index, item);
                    }
        ...
        public static void AddRange<T>(this IList<T> targetCollection, params T[] ts)
        {
            ts.ForEach(T => targetCollection.Add(T));
        }
    }
}